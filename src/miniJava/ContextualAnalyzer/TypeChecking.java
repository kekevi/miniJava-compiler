...
public class TypeChecking implements Visitor<TypeDenoter, TypeDenoter> {
  ...
  @Override
  public TypeDenoter visitFieldDeclInit(FieldDeclInit fd, TypeDenoter arg) {
    if (fd.type.typeKind == TypeKind.VOID) {
      reporter.reportError(prefix(fd.posn) + "void fields are not a thing. How did you get here?");
    }

    check(fd.type, fd.init.visit(this, null), prefix(fd.posn) + "expression value does not match field's type.");
    return fd.type;
  }
  ...
  @Override
  public TypeDenoter visitConstructorDecl(ConstructorDecl cons, TypeDenoter arg) {
    context = new ReturnContext(cons);
    for (Statement statement : cons.sl) {
      statement.visit(this, cons.type); // the cons.type arg is superfluous 
    }
  ...
  public TypeDenoter visitNewObjectExpr(NewObjectExpr expr, TypeDenoter arg) {
    ConstructorDecl constructor = ((ClassDecl) expr.classtype.className.decl).constructor;
    if (constructor.pl.size() != expr.argList.size()) {
      reporter.reportError(prefix(expr.posn) + "invalid number of arguments for the constructor.");
      return null;
    }

    for (int i = 0; i < constructor.pl.size(); i++) {
      TypeDenoter left = constructor.pl.get(i).type;
      TypeDenoter right = expr.argList.get(i).visit(this, null); // maybe pass the expected type too?
      check(left, right, prefix(expr.argList.get(i).posn) + "argument " + i + " has an incorrect type" + suffix(left, right));
    }
...
class ReturnContext {
  ...
  // TODO: this is really unnecessary, instead we should make a new subclass "InvocableDecl" for MDs and CDs, anything that has a ParamList and StatementList
  public ReturnContext (ConstructorDecl cons) {
    this(new MethodDecl(cons, cons.pl, cons.sl, cons.posn));
  }
