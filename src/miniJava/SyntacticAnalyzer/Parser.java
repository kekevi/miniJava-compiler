...
public class Parser {
  ...
  private ClassDecl parseClassDeclaration() throws SyntaxError {
    ...
    ConstructorDecl constructor = null; // NOTE: we only support one constructor per class right now
    // while ("starter set of ClassItem") {
    while (token.kind != TokenKind.RCURLY) {
      MemberSpecifier classItem = parseClassItem();

      switch (classItem.kind) {
        case FIELD:
        ...
        case CONSTRUCTOR:
          if (constructor != null) {
            reporter.reportError("We do not currently support multiple constructors in a class.");
          }
          constructor = ((ConstructorDecl) classItem.classItem);
          break;
      }
    }

    accept(TokenKind.RCURLY);
    return new ClassDecl(classname, fields, methods, constructor, new SourcePosition(class_start, finishpos));
  }
  ...
  class MemberSpecifier {
    ...
    MemberKind kind;
    public MemberSpecifier(MemberDecl classItem, MemberKind kind) {
  ...
  private MemberSpecifier parseClassItem() throws SyntaxError {
    ...
    if (acceptCheck(TokenKind.VOID)) {
      ...
      accept(TokenKind.RCURLY);
      return new MemberSpecifier(new MethodDecl(isPrivate, isStatic, new BaseType(TypeKind.VOID, new SourcePosition(void_pos)), methodname, params, statements, new SourcePosition(classitem_start, finishpos)), MemberKind.METHOD);
    }

    // Token firstId = accept(TokenKind.ID); // either a type or Constructor's class reference
    Token typeOrConsName = acceptIt();

    // ConstructorDecl
    if (acceptCheck(TokenKind.LPAREN)) {
      if (typeOrConsName.kind != TokenKind.ID) {
        parseError(typeOrConsName, "expecting an identifier for a constructor.");
      }
      ...
      accept(TokenKind.RCURLY);
      return new MemberSpecifier(new ConstructorDecl(isPrivate, new ClassType(new Identifier(typeOrConsName), typeOrConsName.posn), typeOrConsName.spelling, params, statements, new SourcePosition(classitem_start, finishpos)), MemberKind.CONSTRUCTOR);
    }

    TypeDenoter type = parseType(typeOrConsName);
    String name = accept(TokenKind.ID).spelling;

    // (typed) FieldDecl
    if (token.kind == TokenKind.SEMICOLON) {
      acceptIt();
      return new MemberSpecifier(new FieldDecl(isPrivate, isStatic, type, name, new SourcePosition(classitem_start, finishpos)), MemberKind.FIELD);
    }

    // (typed) FieldDeclInit (w/ initialization)
    if (token.kind == TokenKind.ASSIGN) {
      acceptIt();
      Expression init = parseExpression();
      accept(TokenKind.SEMICOLON);
      return new MemberSpecifier(new FieldDeclInit(isPrivate, isStatic, type, name, init, new SourcePosition(classitem_start, finishpos)), MemberKind.FIELD);
    }
    ...
    accept(TokenKind.RCURLY);
    return new MemberSpecifier(new MethodDecl(isPrivate, isStatic, type, name, params, statements, new SourcePosition(classitem_start, finishpos)), MemberKind.METHOD);
  }
        ...
        case NEW:
          ...
          if (token.kind == TokenKind.ID) {
            ...
            if (acceptCheck(TokenKind.LPAREN)) {
              ...
              accept(TokenKind.RPAREN);
              return new NewObjectExpr(new ClassType(new Identifier(id), new SourcePosition(type_pos)), args, new SourcePosition(expr_start, finishpos));
            // NewArrayExpr - Object
            } else {
              accept(TokenKind.LSQUARE);
              Expression arraySize = parseExpression();
