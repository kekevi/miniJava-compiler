...
public class ASTDisplay implements Visitor<String,Object> {
    ...
    public Object visitFieldDeclInit(FieldDeclInit f, String arg){
        show(arg, "(" + (f.isPrivate ? "private": "public") 
             + (f.isStatic ? " static) " :") ") + f.toString());
        f.type.visit(this, indent(arg));
        show(indent(arg), quote(f.name) + " fieldname");
        f.init.visit(this, indent(arg));
     return null;
 }
    ...
    public Object visitConstructorDecl(ConstructorDecl c, String arg) {
        show(arg, "(" + (c.isPrivate ? "private": "public") + c.toString());
        c.type.visit(this, indent(arg));
        ParameterDeclList pdl = c.pl;
        show(arg, "  ParameterDeclList [" + pdl.size() + "]");
        String pfx = ((String) arg) + "  . ";
        for (ParameterDecl pd: pdl) {
            pd.visit(this, pfx);
        }
        StatementList sl = c.sl;
        show(arg, "  StmtList [" + sl.size() + "]");
        for (Statement s: sl) {
            s.visit(this, pfx);
        }
